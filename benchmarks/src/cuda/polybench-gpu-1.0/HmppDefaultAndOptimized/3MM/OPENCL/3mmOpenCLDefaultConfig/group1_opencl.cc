...
void hmpp_codelet__threeMMloopa(
  HMPPCG::DataObject* a,
  HMPPCG::DataObject* b,
  HMPPCG::DataObject* e,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  global_size[0] = ((511 / 32LL) + 1LL) * (32LL);
  global_size[1] = ((511 / 8LL) + 1LL) * (8LL);
  size_t local_size[2];
  local_size[0] = 32LL;
  local_size[1] = 8LL;
  a->makeQueueWait(context.cmd_queue);
  b->makeQueueWait(context.cmd_queue);
  e->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[0], 0, sizeof(cl_mem), (void*) &(a->mem_obj));
  clSetKernelArg(context.kernels[0], 1, sizeof(cl_mem), (void*) &(b->mem_obj));
  clSetKernelArg(context.kernels[0], 2, sizeof(cl_mem), (void*) &(e->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[0], 2, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  a->waitOnEvent(kevent);
  b->waitOnEvent(kevent);
  e->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
void hmpp_codelet__threeMMloopb(
  HMPPCG::DataObject* c,
  HMPPCG::DataObject* d,
  HMPPCG::DataObject* f,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  local_size[0] = 32LL;
  local_size[1] = 8LL;
  c->makeQueueWait(context.cmd_queue);
  d->makeQueueWait(context.cmd_queue);
  f->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[1], 0, sizeof(cl_mem), (void*) &(c->mem_obj));
  clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(d->mem_obj));
  clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(f->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[1], 2, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  c->waitOnEvent(kevent);
  d->waitOnEvent(kevent);
  f->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
void hmpp_codelet__threeMMloopc(
  HMPPCG::DataObject* e_1,
  HMPPCG::DataObject* f_1,
  HMPPCG::DataObject* g,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  {
  ...
  local_size[0] = 32LL;
  local_size[1] = 8LL;
  e_1->makeQueueWait(context.cmd_queue);
  f_1->makeQueueWait(context.cmd_queue);
  g->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[2], 0, sizeof(cl_mem), (void*) &(e_1->mem_obj));
  clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(f_1->mem_obj));
  clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(g->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[2], 2, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  e_1->waitOnEvent(kevent);
  f_1->waitOnEvent(kevent);
  g->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
...
typedef struct hmpp_handle_struct
{
  ...
  HMPPCG::DataObject* __arg4;
  HMPPCG::DataObject* __arg5;
  HMPPCG::DataObject* __arg6;
} hmpp_handle_t;
...
{
  __h->context.prog_name = "group1_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__threeMMloopa_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__threeMMloopb_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__threeMMloopc_loop0_");
...
HMPP_API int hmpp_allocateOnHWA(hmpp_handle_t * __h, int major, int minor, const size_t * size, size_t elsize, int dim)
{
  ...
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    {
      __h->__arg0 = __h->context.allocateData(elsize, total, HMPPCG::READ_WRITE);
      if  (!__h->__arg0)
       return -1;
       else
       return 0;
    ...
    {
      __h->__arg4 = __h->context.allocateData(elsize, total, HMPPCG::READ_WRITE);
      if  (!__h->__arg4)
       return -1;
       else
       return 0;
    ...
    {
      __h->__arg5 = __h->context.allocateData(elsize, total, HMPPCG::READ_WRITE);
      if  (!__h->__arg5)
       return -1;
       else
       return 0;
    ...
    {
      __h->__arg6 = __h->context.allocateData(elsize, total, HMPPCG::READ_WRITE);
      if  (!__h->__arg6)
       return -1;
       else
       return 0;
...
HMPP_API int hmpp_readDataFromHWA(hmpp_handle_t * __h, int major, int minor, void * data, const size_t * size, size_t elsize, int dim, int async)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(2,0): // c@hmpp_codelet__threeMMloopb
    {
      return __h->__arg4->readData(data, async != 0);
    }
    case HMPPCG_POP_HASH(2,1): // d@hmpp_codelet__threeMMloopb
    {
      return __h->__arg5->readData(data, async != 0);
    }
    case HMPPCG_POP_HASH(3,2): // g@hmpp_codelet__threeMMloopc
    {
      return __h->__arg6->readData(data, async != 0);
    }
    default: return -1;
  }
}
...
HMPP_API int hmpp_writeDataToHWA(hmpp_handle_t * __h, int major, int minor, const void * data, const size_t * size, size_t elsize, int dim, int async)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(2,0): // c@hmpp_codelet__threeMMloopb
    {
      return __h->__arg4->writeData(data, async != 0);
    }
    case HMPPCG_POP_HASH(2,1): // d@hmpp_codelet__threeMMloopb
    {
      return __h->__arg5->writeData(data, async != 0);
    }
    case HMPPCG_POP_HASH(3,2): // g@hmpp_codelet__threeMMloopc
    {
      return __h->__arg6->writeData(data, async != 0);
    }
    default: return -1;
  }
}
...
HMPP_API int hmpp_readDataSectionFromHWA(hmpp_handle_t * __h, int major, int minor, void * data, const __hmppcg_DataSection *section, const size_t * size, size_t elsize, int dim, int async)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(2,0): // c@hmpp_codelet__threeMMloopb
    {
      return __h->__arg4->sectionCopy(data, section, size, elsize, dim, false, HMPPCG::DataObject::READ);
    }
    case HMPPCG_POP_HASH(2,1): // d@hmpp_codelet__threeMMloopb
    {
      return __h->__arg5->sectionCopy(data, section, size, elsize, dim, false, HMPPCG::DataObject::READ);
    }
    case HMPPCG_POP_HASH(3,2): // g@hmpp_codelet__threeMMloopc
    {
      return __h->__arg6->sectionCopy(data, section, size, elsize, dim, false, HMPPCG::DataObject::READ);
    }
    default: return -1;
  }
}
...
HMPP_API int hmpp_writeDataSectionToHWA(hmpp_handle_t * __h, int major, int minor, const void * data, const __hmppcg_DataSection *section, const size_t * size, size_t elsize, int dim, int async)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(2,0): // c@hmpp_codelet__threeMMloopb
    {
      return __h->__arg4->sectionCopy((void*) data, section, size, elsize, dim , false, HMPPCG::DataObject::WRITE);
    }
    case HMPPCG_POP_HASH(2,1): // d@hmpp_codelet__threeMMloopb
    {
      return __h->__arg5->sectionCopy((void*) data, section, size, elsize, dim , false, HMPPCG::DataObject::WRITE);
    }
    case HMPPCG_POP_HASH(3,2): // g@hmpp_codelet__threeMMloopc
    {
      return __h->__arg6->sectionCopy((void*) data, section, size, elsize, dim , false, HMPPCG::DataObject::WRITE);
    }
    default: return -1;
  }
}
...
HMPP_API int hmpp_waitForWriteTransfer(hmpp_handle_t * __h, int major, int minor)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(2,0): // c@hmpp_codelet__threeMMloopb
    {
      return clFinish(__h->__arg4->cmd);
    }
    case HMPPCG_POP_HASH(2,1): // d@hmpp_codelet__threeMMloopb
    {
      return clFinish(__h->__arg5->cmd);
    }
    case HMPPCG_POP_HASH(3,2): // g@hmpp_codelet__threeMMloopc
    {
      return clFinish(__h->__arg6->cmd);
    }
    default: return -1;
  }
}
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__threeMMloopa(__arg2,__arg3,__arg0)
      hmpp_codelet__threeMMloopa(  __h->__arg2, __h->__arg3, __h->__arg0, status, __h->context);
      return status;
    case 2: // hmpp_codelet__threeMMloopb(__arg4,__arg5,__arg1)
      hmpp_codelet__threeMMloopb(  __h->__arg4, __h->__arg5, __h->__arg1, status, __h->context);
      return status;
    case 3: // hmpp_codelet__threeMMloopc(__arg0,__arg1,__arg6)
      hmpp_codelet__threeMMloopc(  __h->__arg0, __h->__arg1, __h->__arg6, status, __h->context);
      return status;
  }
  return -1;
}
