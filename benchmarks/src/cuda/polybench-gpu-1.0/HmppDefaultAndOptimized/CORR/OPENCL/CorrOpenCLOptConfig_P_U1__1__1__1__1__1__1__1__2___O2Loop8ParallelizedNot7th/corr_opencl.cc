...
void hmpp_codelet__runCorr(
  HMPPCG::DataObject* pdata,
  HMPPCG::DataObject* psymmat,
  HMPPCG::DataObject* pstddev,
  HMPPCG::DataObject* pmean,
  float pfloat_n, float peps, int &hmppcg_status, HMPPCG::CLcontext& context)
{
  {
  cl_event kevent;
  ...
  pdata->makeQueueWait(context.cmd_queue);
  pmean->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[0], 0, sizeof(float), (void*) &pfloat_n);
  clSetKernelArg(context.kernels[0], 1, sizeof(cl_mem), (void*) &(pdata->mem_obj));
  clSetKernelArg(context.kernels[0], 2, sizeof(cl_mem), (void*) &(pmean->mem_obj));
  ...
  pdata->waitOnEvent(kevent);
  pmean->waitOnEvent(kevent);
  ...
  pdata->makeQueueWait(context.cmd_queue);
  pmean->makeQueueWait(context.cmd_queue);
  pstddev->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[1], 0, sizeof(float), (void*) &peps);
  clSetKernelArg(context.kernels[1], 1, sizeof(float), (void*) &pfloat_n);
  clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(pdata->mem_obj));
  clSetKernelArg(context.kernels[1], 3, sizeof(cl_mem), (void*) &(pmean->mem_obj));
  clSetKernelArg(context.kernels[1], 4, sizeof(cl_mem), (void*) &(pstddev->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[1], 1, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  pdata->waitOnEvent(kevent);
  pmean->waitOnEvent(kevent);
  pstddev->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
  {
  ...
  pmean->makeQueueWait(context.cmd_queue);
  pstddev->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[2], 0, sizeof(float), (void*) &pfloat_n);
  clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(pdata->mem_obj));
  clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(pmean->mem_obj));
  clSetKernelArg(context.kernels[2], 3, sizeof(cl_mem), (void*) &(pstddev->mem_obj));
  ...
  ;
  {
    ...
    for (j1_1 = 0, __hmppcg_end = 2046; j1_1 <= __hmppcg_end; j1_1 += 1)
    {
      ...
      cl_event kevent;
      size_t global_size[1];
      global_size[0] = ((0LL / (32LL * 4LL)) + 1LL) * (32LL * 4LL);
      size_t local_size[1];
      local_size[0] = 32LL*4LL;
      psymmat->makeQueueWait(context.cmd_queue);
      int32_t j1_1_1 = j1_1;
      clSetKernelArg(context.kernels[3], 0, sizeof(int32_t), (void*) &j1_1_1);
      clSetKernelArg(context.kernels[3], 1, sizeof(cl_mem), (void*) &(psymmat->mem_obj));
      cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[3], 1, NULL, global_size, local_size, 0, NULL, &kevent);
      if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
      clFlush(context.cmd_queue);
      psymmat->waitOnEvent(kevent);
      clReleaseEvent(kevent);
      }
      ;
      {
      cl_event kevent;
      size_t global_size[1];
      global_size[0] = (((2046 - j1_1) / (32LL * 8LL)) + 1LL) * (32LL * 8LL);
      size_t local_size[1];
      local_size[0] = 32LL*8LL;
      pdata->makeQueueWait(context.cmd_queue);
      psymmat->makeQueueWait(context.cmd_queue);
      int32_t j1_1_2 = j1_1;
      clSetKernelArg(context.kernels[4], 0, sizeof(int32_t), (void*) &j1_1_2);
      clSetKernelArg(context.kernels[4], 1, sizeof(cl_mem), (void*) &(pdata->mem_obj));
      clSetKernelArg(context.kernels[4], 2, sizeof(cl_mem), (void*) &(psymmat->mem_obj));
      cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[4], 1, NULL, global_size, local_size, 0, NULL, &kevent);
      if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
      clFlush(context.cmd_queue);
      pdata->waitOnEvent(kevent);
      psymmat->waitOnEvent(kevent);
  ...
  local_size[0] = 32LL*4LL;
  psymmat->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[5], 0, sizeof(cl_mem), (void*) &(psymmat->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[5], 1, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  psymmat->waitOnEvent(kevent);
  clReleaseEvent(kevent);
...
{
  __h->context.prog_name = "corr_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__runCorr_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__runCorr_loop1_");
  __h->context.kernels_names.push_back("hmpp_codelet__runCorr_loop2_");
  __h->context.kernels_names.push_back("hmpp_codelet__runCorr_loop3_");
  __h->context.kernels_names.push_back("hmpp_codelet__runCorr_loop4_");
  __h->context.kernels_names.push_back("hmpp_codelet__runCorr_loop5_");
...
HMPP_API int hmpp_readDataFromHWA(hmpp_handle_t * __h, int major, int minor, void * data, const size_t * size, size_t elsize, int dim, int async)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(1,4): // pfloat_n@hmpp_codelet__runCorr
    {
      (*(float*)data) = __h->__arg4;
      return 0;
    }
    case HMPPCG_POP_HASH(1,5): // peps@hmpp_codelet__runCorr
    {
      (*(float*)data) = __h->__arg5;
      return 0;
    }
    default: return -1;
  }
...
HMPP_API int hmpp_writeDataToHWA(hmpp_handle_t * __h, int major, int minor, const void * data, const size_t * size, size_t elsize, int dim, int async)
{
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    case HMPPCG_POP_HASH(1,4): // pfloat_n@hmpp_codelet__runCorr
    {
      __h->__arg4 = (*(float *)data);
      return 0;
    }
    case HMPPCG_POP_HASH(1,5): // peps@hmpp_codelet__runCorr
    {
      __h->__arg5 = (*(float *)data);
      return 0;
    }
    default: return -1;
  }
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__runCorr(__arg0,__arg1,__arg2,__arg3,__arg4,__arg5)
      hmpp_codelet__runCorr(  __h->__arg0, __h->__arg1, __h->__arg2, __h->__arg3, __h->__arg4, __h->__arg5, status, __h->context);
      return status;
  }
  return -1;
}
