  ...
  float pfloat_n, float peps, int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  cl_event kevent;
  size_t global_size[1];
  global_size[0] = ((2046 / (32LL * 8LL)) + 1LL) * (32LL * 8LL);
  size_t local_size[1];
  local_size[0] = 32LL*8LL;
  pdata->makeQueueWait(context.cmd_queue);
  psymmat->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[3], 0, sizeof(cl_mem), (void*) &(pdata->mem_obj));
  ...
  psymmat->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[4], 0, sizeof(cl_mem), (void*) &(psymmat->mem_obj));
