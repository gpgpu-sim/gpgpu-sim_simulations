  ...
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  s->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[0], 0, sizeof(cl_mem), (void*) &(s->mem_obj));
  ...
  ;
  {
    ...
    for (i_2 = 0, __hmppcg_end = 4095; i_2 <= __hmppcg_end; i_2 += 1)
    {
      ...
      r->makeQueueWait(context.cmd_queue);
      s->makeQueueWait(context.cmd_queue);
      int32_t i_2_1 = i_2;
      clSetKernelArg(context.kernels[1], 0, sizeof(int32_t), (void*) &i_2_1);
      clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(a->mem_obj));
      clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(r->mem_obj));
      clSetKernelArg(context.kernels[1], 3, sizeof(cl_mem), (void*) &(s->mem_obj));
  ...
  local_size[0] = 32LL*8LL;
  a->makeQueueWait(context.cmd_queue);
  p->makeQueueWait(context.cmd_queue);
  q->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[2], 0, sizeof(cl_mem), (void*) &(a->mem_obj));
  clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(p->mem_obj));
  clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(q->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[2], 1, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  a->waitOnEvent(kevent);
  p->waitOnEvent(kevent);
  q->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
...
{
  __h->context.prog_name = "bicg_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__runBicg_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__runBicg_loop1_");
  __h->context.kernels_names.push_back("hmpp_codelet__runBicg_loop2_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__runBicg(__arg0,__arg1,__arg2,__arg3,__arg4)
      hmpp_codelet__runBicg(  __h->__arg0, __h->__arg1, __h->__arg2, __h->__arg3, __h->__arg4, status, __h->context);
      return status;
  }
  return -1;
}
