...
void hmpp_codelet__runGramSchmidt(
  HMPPCG::DataObject* pA,
  HMPPCG::DataObject* pR,
  HMPPCG::DataObject* pQ,
...
{
  {
    ...
    for (k_1 = 0, __hmppcg_end = 2047; k_1 <= __hmppcg_end; k_1 += 1)
    {
      ...
      pA->makeQueueWait(context.cmd_queue);
      pR->makeQueueWait(context.cmd_queue);
      int32_t k_1_1 = k_1;
      clSetKernelArg(context.kernels[0], 0, sizeof(int32_t), (void*) &k_1_1);
      clSetKernelArg(context.kernels[0], 1, sizeof(cl_mem), (void*) &(pA->mem_obj));
      clSetKernelArg(context.kernels[0], 2, sizeof(cl_mem), (void*) &(pR->mem_obj));
      ...
      pA->waitOnEvent(kevent);
      pR->waitOnEvent(kevent);
      ...
      pA->makeQueueWait(context.cmd_queue);
      pQ->makeQueueWait(context.cmd_queue);
      pR->makeQueueWait(context.cmd_queue);
      int32_t k_1_2 = k_1;
      clSetKernelArg(context.kernels[1], 0, sizeof(int32_t), (void*) &k_1_2);
      clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(pA->mem_obj));
      clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(pQ->mem_obj));
      clSetKernelArg(context.kernels[1], 3, sizeof(cl_mem), (void*) &(pR->mem_obj));
      cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[1], 1, NULL, global_size, local_size, 0, NULL, &kevent);
      if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
      clFlush(context.cmd_queue);
      pA->waitOnEvent(kevent);
      pQ->waitOnEvent(kevent);
      pR->waitOnEvent(kevent);
      clReleaseEvent(kevent);
      }
      ;
      ...
      cl_event kevent;
      size_t global_size[1];
      global_size[0] = (((2046 - k_1) / (32LL * 8LL)) + 1LL) * (32LL * 8LL);
      size_t local_size[1];
      local_size[0] = 32LL*8LL;
      pA->makeQueueWait(context.cmd_queue);
      pQ->makeQueueWait(context.cmd_queue);
      pR->makeQueueWait(context.cmd_queue);
      int32_t k_1_3 = k_1;
      clSetKernelArg(context.kernels[2], 0, sizeof(int32_t), (void*) &k_1_3);
      clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(pA->mem_obj));
      clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(pQ->mem_obj));
      clSetKernelArg(context.kernels[2], 3, sizeof(cl_mem), (void*) &(pR->mem_obj));
...
{
  __h->context.prog_name = "gramschmidt_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__runGramSchmidt_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__runGramSchmidt_loop1_");
  __h->context.kernels_names.push_back("hmpp_codelet__runGramSchmidt_loop2_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__runGramSchmidt(__arg0,__arg1,__arg2)
      hmpp_codelet__runGramSchmidt(  __h->__arg0, __h->__arg1, __h->__arg2, status, __h->context);
      return status;
  }
  return -1;
}
