...
//   for (i = 1 ; i < 4096 - 1 ; ++i)
//     {
// #pragma hmppcg unroll 8, split, guarded
// # 22 "<preprocessor>"
// # 47 "twodconv.c"
//       for (j = 1 ; j < 4096 - 1 ; ++j)
//         {
  ...
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  global_size[0] = ((516 / 32LL) + 1LL) * (32LL);
  global_size[1] = ((4093 / 8LL) + 1LL) * (8LL);
  size_t local_size[2];
  local_size[0] = 32LL;
  local_size[1] = 8LL;
...
{
  __h->context.prog_name = "conv_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__conv2D_loop0_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__conv2D(__arg0,__arg1)
      hmpp_codelet__conv2D(  __h->__arg0, __h->__arg1, status, __h->context);
      return status;
  }
  return -1;
}
