  ...
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  global_size[0] = ((1023 / 32LL) + 1LL) * (32LL);
  global_size[1] = ((2047 / 8LL) + 1LL) * (8LL);
  size_t local_size[2];
  local_size[0] = 32LL;
  local_size[1] = 8LL;
  c->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[0], 0, sizeof(cl_mem), (void*) &(c->mem_obj));
  ...
  clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(b->mem_obj));
  clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(c->mem_obj));
...
{
  __h->context.prog_name = "syrtwok_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__runSyrTwoK_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__runSyrTwoK_loop1_");
...
HMPP_API int hmpp_allocateOnHWA(hmpp_handle_t * __h, int major, int minor, const size_t * size, size_t elsize, int dim)
{
  ...
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    {
      __h->__arg1 = __h->context.allocateData(elsize, total, HMPPCG::READ_ONLY);
      if  (!__h->__arg1)
       return -1;
       else
       return 0;
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__runSyrTwoK(__arg0,__arg1,__arg2)
      hmpp_codelet__runSyrTwoK(  __h->__arg0, __h->__arg1, __h->__arg2, status, __h->context);
      return status;
  }
  return -1;
}
