...
void hmpp_codelet__covarLoopa(
  HMPPCG::DataObject* pmean,
  HMPPCG::DataObject* pdata,
  float pfloat_n, int &hmppcg_status, HMPPCG::CLcontext& context)
{
  {
  cl_event kevent;
...
void hmpp_codelet__covarLoopb(
  HMPPCG::DataObject* pdata_2,
  HMPPCG::DataObject* pmean_1,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  local_size[0] = 32LL;
  local_size[1] = 8LL;
  pdata_2->makeQueueWait(context.cmd_queue);
  pmean_1->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[1], 0, sizeof(cl_mem), (void*) &(pdata_2->mem_obj));
  clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(pmean_1->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[1], 2, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  pdata_2->waitOnEvent(kevent);
  pmean_1->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
void hmpp_codelet__covarLoopc(
  HMPPCG::DataObject* psymmat,
  HMPPCG::DataObject* pdata_1,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  {
    ...
    for (j1_2 = 0, __hmppcg_end = 2047; j1_2 <= __hmppcg_end; j1_2 += 1)
    {
      ...
      cl_event kevent;
      size_t global_size[1];
      global_size[0] = (((2047 - j1_2) / (32LL * 8LL)) + 1LL) * (32LL * 8LL);
      size_t local_size[1];
      local_size[0] = 32LL*8LL;
      pdata_1->makeQueueWait(context.cmd_queue);
      psymmat->makeQueueWait(context.cmd_queue);
      int32_t j1_2_1 = j1_2;
      clSetKernelArg(context.kernels[2], 0, sizeof(int32_t), (void*) &j1_2_1);
      clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(pdata_1->mem_obj));
      clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(psymmat->mem_obj));
      cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[2], 1, NULL, global_size, local_size, 0, NULL, &kevent);
      if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
      clFlush(context.cmd_queue);
      pdata_1->waitOnEvent(kevent);
...
HMPP_API int hmpp_setOpenCLDevice(hmpp_handle_t * __h, cl_context ctx, cl_device_id dev)
{
  ...
  __h->context.kernels_names.push_back("hmpp_codelet__covarLoopa_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__covarLoopb_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__covarLoopc_loop0_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__covarLoopa(__arg0,__arg1,__arg2)
      hmpp_codelet__covarLoopa(  __h->__arg0, __h->__arg1, __h->__arg2, status, __h->context);
      return status;
    case 2: // hmpp_codelet__covarLoopb(__arg1,__arg0)
      hmpp_codelet__covarLoopb(  __h->__arg1, __h->__arg0, status, __h->context);
      return status;
    case 3: // hmpp_codelet__covarLoopc(__arg3,__arg1)
      hmpp_codelet__covarLoopc(  __h->__arg3, __h->__arg1, status, __h->context);
      return status;
  }
  return -1;
}
