  ...
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  global_size[1] = ((1023 / 8LL) + 1LL) * (8LL);
  size_t local_size[2];
  local_size[0] = 32LL;
  local_size[1] = 8LL;
  ...
  clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(c->mem_obj));
...
{
  __h->context.prog_name = "syrk_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__runSyrk_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__runSyrk_loop1_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__runSyrk(__arg0,__arg1)
      hmpp_codelet__runSyrk(  __h->__arg0, __h->__arg1, status, __h->context);
      return status;
  }
  return -1;
}
