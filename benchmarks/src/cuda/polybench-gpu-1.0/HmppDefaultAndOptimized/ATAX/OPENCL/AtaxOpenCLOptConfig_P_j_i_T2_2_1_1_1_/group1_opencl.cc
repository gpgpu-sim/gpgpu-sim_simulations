...
//
// #pragma hmppcg cpiparam __arg0 INOUT a%hmpp_codelet__ataxLoopa: (1, 0) a%hmpp_codelet__ataxLoopb: (2, 0)
...
// #pragma hmppcg cpiparam __arg3 INOUT x%hmpp_codelet__ataxLoopa: (1, 1)
// 
...
void hmpp_codelet__ataxLoopa(
  HMPPCG::DataObject* a,
  HMPPCG::DataObject* x,
  HMPPCG::DataObject* y,
  HMPPCG::DataObject* tmp,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  {
  cl_event kevent;
  size_t global_size[2];
  global_size[0] = ((1 / 32LL) + 1LL) * (32LL);
  global_size[1] = ((2047 / 4LL) + 1LL) * (4LL);
  size_t local_size[2];
  local_size[0] = 32LL;
  local_size[1] = 4LL;
  y->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[0], 0, sizeof(cl_mem), (void*) &(y->mem_obj));
  ...
  tmp->makeQueueWait(context.cmd_queue);
  x->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[1], 0, sizeof(cl_mem), (void*) &(a->mem_obj));
  clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(tmp->mem_obj));
  clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(x->mem_obj));
  ...
  tmp->waitOnEvent(kevent);
  x->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
void hmpp_codelet__ataxLoopb(
  HMPPCG::DataObject* a_1,
  HMPPCG::DataObject* y_1,
  HMPPCG::DataObject* tmp_1,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  {
  cl_event kevent;
  size_t global_size[1];
  global_size[0] = ((4095 / (32LL * 8LL)) + 1LL) * (32LL * 8LL);
  size_t local_size[1];
  local_size[0] = 32LL*8LL;
  a_1->makeQueueWait(context.cmd_queue);
  tmp_1->makeQueueWait(context.cmd_queue);
  y_1->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[2], 0, sizeof(cl_mem), (void*) &(a_1->mem_obj));
  clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(tmp_1->mem_obj));
  clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(y_1->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[2], 1, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  a_1->waitOnEvent(kevent);
  tmp_1->waitOnEvent(kevent);
  y_1->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
...
HMPP_API int hmpp_setOpenCLDevice(hmpp_handle_t * __h, cl_context ctx, cl_device_id dev)
{
  ...
  __h->context.kernels_names.push_back("hmpp_codelet__ataxLoopa_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__ataxLoopa_loop1_");
  __h->context.kernels_names.push_back("hmpp_codelet__ataxLoopb_loop0_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__ataxLoopa(__arg0,__arg3,__arg1,__arg2)
      hmpp_codelet__ataxLoopa(  __h->__arg0, __h->__arg3, __h->__arg1, __h->__arg2, status, __h->context);
      return status;
    case 2: // hmpp_codelet__ataxLoopb(__arg0,__arg1,__arg2)
      hmpp_codelet__ataxLoopb(  __h->__arg0, __h->__arg1, __h->__arg2, status, __h->context);
      return status;
  }
  return -1;
}
