  ...
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  cl_event kevent;
  size_t global_size[1];
  global_size[0] = ((4095 / (32LL * 4LL)) + 1LL) * (32LL * 4LL);
  size_t local_size[1];
...
{
  {
    ...
    for (i_4 = 0, __hmppcg_end = 4095; i_4 <= __hmppcg_end; i_4 += 1)
    {
      ...
      int32_t i_4_1 = i_4;
      clSetKernelArg(context.kernels[2], 0, sizeof(int32_t), (void*) &i_4_1);
      clSetKernelArg(context.kernels[2], 1, sizeof(cl_mem), (void*) &(a_1->mem_obj));
      clSetKernelArg(context.kernels[2], 2, sizeof(cl_mem), (void*) &(tmp_1->mem_obj));
      clSetKernelArg(context.kernels[2], 3, sizeof(cl_mem), (void*) &(y_1->mem_obj));
...
HMPP_API int hmpp_allocateOnHWA(hmpp_handle_t * __h, int major, int minor, const size_t * size, size_t elsize, int dim)
{
  ...
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    {
      __h->__arg3 = __h->context.allocateData(elsize, total, HMPPCG::READ_ONLY);
      if  (!__h->__arg3)
       return -1;
       else
       return 0;
