...
void hmpp_codelet__runMvt(
  HMPPCG::DataObject* a,
  HMPPCG::DataObject* x1,
  HMPPCG::DataObject* x2,
  HMPPCG::DataObject* y1,
  HMPPCG::DataObject* y2,
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  x1->makeQueueWait(context.cmd_queue);
  y1->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[0], 0, sizeof(cl_mem), (void*) &(a->mem_obj));
  clSetKernelArg(context.kernels[0], 1, sizeof(cl_mem), (void*) &(x1->mem_obj));
  clSetKernelArg(context.kernels[0], 2, sizeof(cl_mem), (void*) &(y1->mem_obj));
  ...
  x1->waitOnEvent(kevent);
  y1->waitOnEvent(kevent);
  ...
  a->makeQueueWait(context.cmd_queue);
  x2->makeQueueWait(context.cmd_queue);
  y2->makeQueueWait(context.cmd_queue);
  clSetKernelArg(context.kernels[1], 0, sizeof(cl_mem), (void*) &(a->mem_obj));
  clSetKernelArg(context.kernels[1], 1, sizeof(cl_mem), (void*) &(x2->mem_obj));
  clSetKernelArg(context.kernels[1], 2, sizeof(cl_mem), (void*) &(y2->mem_obj));
  cl_int err = clEnqueueNDRangeKernel(context.cmd_queue, context.kernels[1], 1, NULL, global_size, local_size, 0, NULL, &kevent);
  if(CHECK_STATUS(err)) {hmppcg_status = -1; return;}
  clFlush(context.cmd_queue);
  a->waitOnEvent(kevent);
  x2->waitOnEvent(kevent);
  y2->waitOnEvent(kevent);
  clReleaseEvent(kevent);
  }
  ;
} 
...
{
  __h->context.prog_name = "mvt_opencl.cc-kernels.cl";
  __h->context.kernels_names.push_back("hmpp_codelet__runMvt_loop0_");
  __h->context.kernels_names.push_back("hmpp_codelet__runMvt_loop1_");
...
HMPP_API int hmpp_allocateOnHWA(hmpp_handle_t * __h, int major, int minor, const size_t * size, size_t elsize, int dim)
{
  ...
  switch(HMPPCG_POP_HASH(major,minor))
  {
    ...
    {
      __h->__arg4 = __h->context.allocateData(elsize, total, HMPPCG::READ_ONLY);
      if  (!__h->__arg4)
       return -1;
       else
       return 0;
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__runMvt(__arg0,__arg1,__arg2,__arg3,__arg4)
      hmpp_codelet__runMvt(  __h->__arg0, __h->__arg1, __h->__arg2, __h->__arg3, __h->__arg4, status, __h->context);
      return status;
  }
  return -1;
}
