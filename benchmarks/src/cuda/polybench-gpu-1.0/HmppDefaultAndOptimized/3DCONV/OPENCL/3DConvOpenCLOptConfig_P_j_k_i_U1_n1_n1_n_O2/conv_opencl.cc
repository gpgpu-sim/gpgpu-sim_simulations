  ...
  int &hmppcg_status, HMPPCG::CLcontext& context)
{
  ...
  global_size[0] = ((253 / 32LL) + 1LL) * (32LL);
  global_size[1] = ((253 / 8LL) + 1LL) * (8LL);
  size_t local_size[2];
  local_size[0] = 32LL;
  local_size[1] = 8LL;
...
HMPP_API int hmpp_setOpenCLDevice(hmpp_handle_t * __h, cl_context ctx, cl_device_id dev)
{
  ...
  __h->context.kernels_names.push_back("hmpp_codelet__conv3D_loop0_");
...
HMPP_API int hmpp_start(hmpp_handle_t * __h, int __id, int __async)
{
  ...
  switch(__id) { 
    case 1: // hmpp_codelet__conv3D(__arg0,__arg1)
      hmpp_codelet__conv3D(  __h->__arg0, __h->__arg1, status, __h->context);
      return status;
  }
  return -1;
}
