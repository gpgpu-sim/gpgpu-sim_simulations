...
// a CUDA host call returns an error.
...
inline void __checkCudaErrors(cudaError err, 
    ...
    if (cudaSuccess != err) {
        fprintf(stderr, "%s(%i) : CUDA Runtime API error %d: %s.\n",
                file, line, static_cast<int> (err), cudaGetErrorString(err));
        exit(-1);
    }
}
...
public:
    ...
    chunkSize_(chunkSize) {
        basePtr_ = thrust::device_malloc<T>(chunkSize * chunksCount);
        
        for (uint chunkIndex = 0; chunkIndex < chunksCount; ++chunkIndex) {
            chunks_.push_back(basePtr_ + chunkSize * chunkIndex);
        }
    }
...
public:
    ...
    void dump(uint width, uint height) const {
        ...
        uint requiredDigitsCount = 
            log10(static_cast<float> (levelsCount())) + 1;
...
private:
    ...
    public:
        ...
        void buildFromDeviceData(thrust::device_ptr<uint> superVerticesOffsets,
            ...
            checkCudaErrors(cudaMemcpy(&(nodes_[0]), 
                                       verticesIDs.get(), 
    ...
    private:
        ...
        // The pair of the following vectors describes the relation between
        // the consecutive levels.
    ...
    void dumpLevel(LevelsIterator level, 
        ...
        uint levelIndex = std::distance(levels_.rbegin(), level);
        ...
        
        {
            ...
            totalSegments = superNodesOffsets.size();
            ...
            for (uint superNodeIndex = 0, nodeIndex = 0; 
                ...
                uint superNodeEnd = 
                superNodeIndex < superNodesOffsets.size() - 1 ?
        ...
        while (level != levels_.rend()) {
            uint superNodesCount = nodesQueue.size();
            ...
            while (superNodesCount--) {
                ...
                uint superNodeEnd = 
                currentNode.first < superNodesOffsets.size() - 1 ?
...
public:
    ...
    float run(const Graph &graph, Pyramid &segmentations) {  
        ...
        MemoryPoolsCollection pools = {
            DeviceMemoryPool<uint>(graph.vertices.size(), 
                                   kUintVerticesPoolsRequired),
            DeviceMemoryPool<float>(graph.vertices.size(), 
                                    kFloatVerticesPoolsRequired),
            DeviceMemoryPool<uint>(graph.edges.size(), 
                                   kUintEdgesPoolsRequired),
            DeviceMemoryPool<float>(graph.edges.size(), 
...
private:
    ...
    void initalizeData(const Graph &graph, MemoryPoolsCollection &pools) {
        ...
        verticesCount_ = graph.vertices.size();
        edgesCount_ = graph.edges.size();
        
        dVertices_ = pools.uintVertices.get();
        dEdges_ = pools.uintEdges.get();
        dWeights_ = pools.floatEdges.get();
        
        dThresholds_ = pools.floatVertices.get();
        dAreas_ = pools.uintVertices.get();
        ...
        thrust::fill(dAreas_, dAreas_ + verticesCount_, 1);
        thrust::fill(dThresholds_, dThresholds_ + verticesCount_, 0);
    }
    ...
    AlgorithmStatus invokeStep(MemoryPoolsCollection &pools, 
        ...
        // To make things clear. Let "dEdgesFlags" denote groups of edges that
        ...
        thrust::sort(
            thrust::make_zip_iterator(
                thrust::make_tuple(dSuccessors,
                                   dClusteredVerticesIDs)),
            thrust::make_zip_iterator(
                thrust::make_tuple(dSuccessors + verticesCount_,
                                   dClusteredVerticesIDs + verticesCount_)));
        ...
        uint *invalidEdgesPtr = 
            thrust::find_if(dNewStartpoints,
        ...
        uint validEdgesCount = invalidEdgesPtr - dNewStartpoints.get(); 
        ...
        // as we already have "dVerticesMapping" that contains all sufficient
        // information about the vertices' transformations.
    ...
    thrust::device_ptr<float> dThresholds_;
    thrust::device_ptr<uint> dAreas_;
...
{ 
    ...
    if ((deviceProp.major > 1) || (deviceProp.major == 1 && deviceProp.minor >= 3))
    {
        ...
        cudaDeviceReset();
        shrQAFinishExit(argc, (const char **)argv, (bResults[0] && bResults[1]) ? QA_PASSED : QA_FAILED);
    } else {
        printf("segmentationTreeThrust requires a GPU with Compute Capability 1.3 or higher, exiting...\n\n");
        cudaDeviceReset();
