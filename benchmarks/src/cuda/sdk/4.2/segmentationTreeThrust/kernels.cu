...
template <typename T>
struct CalculateThreshold : public thrust::binary_function<T, T, T> {
    __host__ __device__ CalculateThreshold(const T &k) : k_(k) {}
    
    __host__ __device__ T operator()(const T &internalDifference,
                                     const T &area) const {
        return internalDifference + k_ / area;
    }
