...
StopWatchInterface *timer_kernel;
               ...
               int radius, int iterations, int nthreads)
{
    ...
    for(int i=0; i<iterations; i++) 
    {
        ...
        checkCudaErrors( cudaDeviceSynchronize() );
        dKernelTime = sdkGetTimerValue(&timer_kernel);
    ...
    sdkStopTimer(&timer_kernel);
    sdkDeleteTimer(&timer_kernel);
